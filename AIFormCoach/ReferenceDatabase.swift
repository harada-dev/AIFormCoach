...
enum ReferenceDatabase {
    ...
    static let instepShot: [Metric] = [
        ...
        Metric(
            ...
            source: "PRD v1.2 §2 の例示値(要監修)。Phase 0 実測で本気のキック時に内角106°を確認。文献(Cordeiro et al. 2015 / Greig 2018〈二次引用〉)を内角に換算すると 68〜87° で例示値の窓 90〜110° と重ならず、また角度の最適値を支持する研究が無いため、基準値未確定として処方対象外とした",
            confidence: .undetermined,
            angleDefinitionVersion: 2,
            requiresSideView: false,
            coachingNote: "振り上げでいったん膝をたたみ、そこから振り出すのはインステップキックに共通のポイントです。ただし「何度が正解」かを言える根拠は現時点でありません(研究ではボール速度に効くのは角度ではなくインパクト時の膝を伸ばす速さでした)。形を覚える練習としては、ボールなしで、振り上げたときにかかとがおしりに近づくくらい曲げる素振りを20回。速く振るより形を作るのが先です。振り出しが遅れる感じがあるときは「イチ、ニ」の2拍で振り上げから振り下ろしまで通す素振りを20回、同じテンポで行います。",
            whenAbove: nil,
            whenBelow: nil
        ),
        ...
        Metric(
            ...
            source: "PRD v1.2 §2 の例示値(要監修)。Phase 0 実測でしっかり蹴った際に15°を確認。文献値は −5.8 ± 8.3°(Alcock et al. 2012、対鉛直グローバル角=後傾)と 41.4 ± 8.8°(脊椎相対屈曲角、二次引用)で、どちらも 15〜20° の近傍になく定義も両立しない。加えて現在の計測は符号を持たず前傾・後傾・側屈を区別できないため、基準値未確定として処方対象外とした",
            confidence: .undetermined,
            angleDefinitionVersion: 2,
            requiresSideView: false,
            coachingNote: "軸足をボールの横にしっかり踏み込み、上体を軸足の上に預けるのはインステップキックに共通のポイントです。ただし「前傾何度が正解」かを言える根拠は現時点でありません。練習としては、軸足を置く位置に印をつけ、そこへ強く踏み込んでから振る素振りを20回。逆に前へ突っ込む癖があるときは、壁から一歩離れて立ち、頭が壁に触れない範囲で振る素振りを20回行います。",
            whenAbove: nil,
            whenBelow: nil
        ),
